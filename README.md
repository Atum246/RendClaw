...
| `GITHUB_GIST_TOKEN` | ⚡ **Required!** GitHub token with `gist` scope — backs up your workspace automatically |

4. Wait for deploy → Done! 🎉

> ⚠️ **Without `GITHUB_GIST_TOKEN`**, your workspace (conversations, memory, config) will be **lost on every restart**. Render free tier has no persistent storage — this is your lifeline! Get a token at [GitHub Settings → Developer Settings → Personal Access Tokens](https://github.com/settings/tokens) with `gist` scope.
